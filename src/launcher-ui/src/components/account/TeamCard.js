...
const TeamCard = ({ team, onUpdateTeam }) => {
  ...
  const fetchGames = useMemo(
    () => async () => {
      ...
      try {
        const response = await fetch(`/get-team-games?team_name=${encodeURIComponent(team.team_name)}`);
  ...
  return (
    <li className={'gap-3 flex flex-col justify-between border rounded-sm'} style={listStyle}>
      <Stack className={'h-full gap-3 p-3 justify-between flex-col flex'}>
        ...
        <Stack flexDirection='row' justifyContent='space-between' alignItems='center' spacing={'12px'}>
          <Stack flexDirection='row' alignItems='center' gap='12px'>
            <Avatar src={`${team.team_icon_url}?t=${Date.now()}`} alt={team.team_name} variant='square' sx={{ width: 32, height: 32, '& img': { objectFit: 'scale-down' } }} />
        ...
        {loadingGames ? <InfiniteGameSkeleton /> : errorGames ? <p style={{ color: 'red', textAlign: 'center' }}>{errorGames}</p> : <InfiniteGameScroller games={games} />}
      ...
      {/* Edit Team Dialog */}
      <EditTeamDialog open={editOpen} handleClose={() => setEditOpen(false)} team={team} onSave={handleSaveTeamChanges} />
    </li>
  );
};
